...
test("correctly renders styling", () => {
  const hello = jest.fn();
  const {getByTestId} = render(<Input type="number" onChange={hello} />);
  expect(getByTestId("input-box")).toHaveClass("shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline");
});

test("on change gets called", async () => {
  const hello = jest.fn();
  const {getByTestId} = render(<Input type="text" onChange={hello} />);
